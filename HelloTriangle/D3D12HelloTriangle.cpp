...
void D3D12HelloTriangle::OnInit()
{
    ...
    // Setup AS for raytracing. When setting up geometry, each bottom-level AS has its own transform matrix.
    CreateAccelerationStructures();
    // Create the raytracing pipeline, associating the shader code to symbol names and to their root signatures,
    // and defining the amount of memory carried by rays (ray payload)
...
void D3D12HelloTriangle::LoadPipeline()
{
    ...
    if (m_useWarpDevice)
    {
        ...
        ThrowIfFailed(D3D12CreateDevice(warpAdapter.Get(), D3D_FEATURE_LEVEL_11_1, IID_PPV_ARGS(&m_device)));
    }
    else
    {
...
void D3D12HelloTriangle::CheckRaytracingSupport()
{
    ...
    {
        throw std::runtime_error("Raytracing isn't supported on device");
    }
}
...
void D3D12HelloTriangle::OnKeyUp(UINT8 key)
{
    if (key == VK_SPACE)
    {
        m_rasterEnabled = !m_rasterEnabled;
    }
}
...
void D3D12HelloTriangle::PopulateCommandList()
{
    ...
    else // DXR
    {
        ...
        // The layout of the SBT is as follows: ray generation shader, miss shaders, hit groups.
        // As described in the CreateShaderBindingTable method, all SBT entries of a given type have the same size to allow a fixed stride.
        ...
        raysDesc.MissShaderTable.StartAddress = m_sbtStorage->GetGPUVirtualAddress() + rayGenerationSectionSizeInBytes;
        ...
        raysDesc.HitGroupTable.StartAddress = m_sbtStorage->GetGPUVirtualAddress() + rayGenerationSectionSizeInBytes + missSectionSizeInBytes;
        raysDesc.HitGroupTable.SizeInBytes = hitGroupSectionSizeInBytes;
        ...
        // For this, we need to transition the raytracing output from a UAV to a copy source, and the render target buffer to a copy destination.
...
void D3D12HelloTriangle::CreateAccelerationStructures()
{
    ...
    AccelerationStructureBuffers bottomLevelBuffers = CreateBottomLevelAS({{m_vertexBuffer.Get(), 3u}});
...
D3D12HelloTriangle::AccelerationStructureBuffers D3D12HelloTriangle::CreateBottomLevelAS(std::vector<std::pair<ComPtr<ID3D12Resource>, uint32_t>> vVertexBuffers)
{
    ...
    UINT64 scratchSizeInBytes = (UINT64)0u;
    // The final AS also needs to be stored in addition to the existing vertex
    // buffers. It size is also dependent on the scene complexity.
    UINT64 resultSizeInBytes = (UINT64)0u;

    bottomLevelAS.ComputeASBufferSizes(m_device.Get(), false, &scratchSizeInBytes, &resultSizeInBytes);

    // Once the sizes are obtained, the application is responsible for allocating the necessary buffers.
    // Since the entire generation will be done on the GPU, we can directly allocate those on the default heap
...
// Similarly to the bottom-level AS generation, it is done in 3 steps: gathering the instances,
// computing the memory requirements for the AS, and building the AS itself
    ...
    const std::vector<std::pair<ComPtr<ID3D12Resource>, DirectX::XMMATRIX>> &instances) // pair of bottom level AS and matrix of the instance
{
    ...
    for (size_t i = 0; i < instances.size(); i++)
    {
        m_topLevelASGenerator.AddInstance(
            instances[i].first.Get(), // BottomLevelAS
            instances[i].second,      // Matrix of the instance
            static_cast<UINT>(i),     // instanceID
            static_cast<UINT>(0u));   // hitGroupID
    }
    ...
    // This call outputs the memory requirements for each (scratch, results, instance descriptors) so that the application can allocate the corresponding memory
    ...
    m_topLevelASBuffers.pScratch = nv_helpers_dx12::CreateBuffer(
        ...
        D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, // D3D12_RESOURCE_STATE_COMMON probably should be
        nv_helpers_dx12::kDefaultHeapProps);
    ...
    // Those will be copied into the buffer by the helper through mapping, so the buffer has to be allocated on the upload heap.
    ...
    m_topLevelASGenerator.Generate(m_commandList.Get(),
                                   ...
                                   m_topLevelASBuffers.pInstanceDesc.Get(), false, nullptr /*previous AS*/);
}

//-----------------------------------------------------------------------------
// The ray generation shader needs to access 2 resources: the raytracing output and the top-level acceleration structure
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateRayGenRootSignature()
{
    nv_helpers_dx12::RootSignatureGenerator rsc;
    rsc.AddHeapRangesParameter({
        {SHADER_REGISTER(0u) /*u0*/, NUM_DESCRIPTORS(1u), REGISTER_SPACE(0u), D3D12_DESCRIPTOR_RANGE_TYPE_UAV /*Output*/, OFFSET(0u)},
        {SHADER_REGISTER(0u) /*t0*/, NUM_DESCRIPTORS(1u), REGISTER_SPACE(0u), D3D12_DESCRIPTOR_RANGE_TYPE_SRV /*TLAS*/, OFFSET(1u)},
    });
...
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateHitRootSignature()
{
    nv_helpers_dx12::RootSignatureGenerator rsc;
    rsc.AddRootParameter(D3D12_ROOT_PARAMETER_TYPE_SRV, SHADER_REGISTER(0u), REGISTER_SPACE(0u));
...
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateMissRootSignature()
{
    nv_helpers_dx12::RootSignatureGenerator rsc;
    return rsc.Generate(m_device.Get(), true);
}
...
// characteristics in a single structure used by DXR to invoke the shaders and manage temporary memory during raytracing
void D3D12HelloTriangle::CreateRaytracingPipeline()
{
    ...
    // Note that a single library can contain an arbitrary number of symbols, whose semantic is given in HLSL
    ...
    m_rayGenSignature = CreateRayGenRootSignature();
    m_missSignature = CreateMissRootSignature();
    m_hitSignature = CreateHitRootSignature();

    // 3 different shaders can be invoked to obtain an intersection:
    // an intersection shader is called when hitting the bounding box of non-triangular geometry.
    // An any-hit shader is called on potential intersections.
    // This shader can, for example, perform alpha-testing and discard some intersections.
    // Finally, the closest-hit program is invoked on the intersection point closest to the ray origin.
    // Those 3 shaders are bound together into a hit group.

    // Note that for triangular geometry the intersection shader is built-in.
    // An empty any-hit shader is also defined by default, so in our simple case each hit group contains only the closest hit shader.
    // Note that since the exported symbols are defined above the shaders can be simply referred to by name.

    // Hit group for the triangles, with a shader simply interpolating vertex colors
    pipeline.AddHitGroup(L"HitGroup", L"ClosestHit");

    // The following section associates the root signature to each shader. Note that we can explicitly show that some shaders share the same root signature (eg. Miss and ShadowMiss).
    // Note that the hit shaders are now only referred to as hit groups, meaning that the underlying intersection, any-hit and closest-hit shaders share the same root signature.
    pipeline.AddRootSignatureAssociation(m_rayGenSignature.Get(), {L"RayGen"});
    pipeline.AddRootSignatureAssociation(m_missSignature.Get(), {L"Miss"});
    pipeline.AddRootSignatureAssociation(m_hitSignature.Get(), {L"HitGroup"});

    // Data carried by the rays
    pipeline.SetMaxPayloadSize(4u * sizeof(float)); // RGB + distance
    // Upon hitting a surface, DXR can provide several attributes to the hit. In
    // our sample we just use the barycentric coordinates defined by the weights
    // u,v of the last two vertices of the triangle. The actual barycentrics can
    // be obtained using float3 barycentrics = float3(1.f-u-v, u, v);
    pipeline.SetMaxAttributeSize(2u * sizeof(float)); // barycentric coordinates
    // The raytracing process can shoot rays from existing hit points, resulting
    // in nested TraceRay calls. Our sample code traces only primary rays, which
    // then requires a trace depth of 1. Note that this recursion depth should be
    // kept to a minimum for best performance. Path tracing algorithms can be
    // easily flattened into a simple loop in the ray generation.
    pipeline.SetMaxRecursionDepth(1u);
...
void D3D12HelloTriangle::CreateRaytracingOutputBuffer()
{
    ...
    resDesc.DepthOrArraySize = (UINT16)1u;
    resDesc.MipLevels = (UINT16)1u;
    resDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    resDesc.SampleDesc = {1u, 0u};
    resDesc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
    resDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    ThrowIfFailed(m_device->CreateCommittedResource(&nv_helpers_dx12::kDefaultHeapProps, D3D12_HEAP_FLAG_NONE, 
        &resDesc, D3D12_RESOURCE_STATE_COPY_SOURCE, nullptr, IID_PPV_ARGS(m_outputResource.GetAddressOf())));
}

#define SHADER_VISIBLE (TRUE)
...
void D3D12HelloTriangle::CreateShaderResourceHeap()
{
    ...
    m_srvUavHeap = nv_helpers_dx12::CreateDescriptorHeap(m_device.Get(), 2u, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, SHADER_VISIBLE);
...
void D3D12HelloTriangle::CreateShaderBindingTable()
{
    ...
    // The helper treats both root parameter pointers and heap pointers as void*, while DX12 uses the D3D12_GPU_DESCRIPTOR_HANDLE to define heap pointers.
