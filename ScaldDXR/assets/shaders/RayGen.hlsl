...
void RayGen() 
{
    ...
    HitInfo payload;
    payload.colorAndDistance = 0.f.xxxx;
    ...
    float2 uv = ((launchIndex.xy + 0.5f /*like jitter or smth*/) / dims.xy) * float2(2.0f, -2.0f) + float2(-1.0f, 1.0f);
    
    // Define a ray, consisting of origin, direction, and the min-max distance values
    RayDesc ray;
    ray.Origin = float3(uv.x, uv.y, 1.0f);
    ray.Direction = float3(0.0f, 0.0f, -1.0f);
    // unlike rasterization, the ratio between TMin and TMax does not have any impact on the precision of the intersection calculations
    ray.TMin = 0.1f;
    ray.TMax = FLT_MAX;
